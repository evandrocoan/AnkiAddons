...
target_note_type_name = "Some Note"

value_replace = r"""<img src="\1">"""

replace_search = re.compile(r"""<\s*img\s*src\s*=\s*"(.*?)"\s*\/\s*>""")

condition_field = r"Image"
condition_search = re.compile(r"""<\s*img\s*src\s*=\s*"(.*?)"\s*\/\s*>""")
